...
let ch1_oms = (() => {
    ...
    let make_tangle_consumer = (begin_enabled = true) => venience_world_1.wrap_handler(function* (parser) {
        ...
        let consider_consumer = venience_world_1.wrap_handler(function* (parser) {
            ...
            let choice_str = yield parser.consume_option([
                datatypes_1.set_enabled(['first', 'fragment'], !this.state.has_understood['tangle, 1'] && prev_contention !== 'tangle, 1'),
                datatypes_1.set_enabled(['second', 'fragment'], !this.state.has_understood['tangle, 2'] && prev_contention !== 'tangle, 2'),
                datatypes_1.set_enabled(['third', 'fragment'], !this.state.has_understood['tangle, 3'] && prev_contention !== 'tangle, 3')
            ]);
        ...
        return parser_1.combine.call(this, parser, [
            begin_consumer,
            reify_consumer,
            consider_consumer,
            end_consumer
            ...
            handle_command: venience_world_1.wrap_handler(function* (parser) {
                ...
                let go_consumer = venience_world_1.wrap_handler(function* (parser) {
                    ...
                    let dir = yield parser.consume_option([
                        ...
                        ['south?'],
                        ['west?']
                    ].map(d => datatypes_1.set_enabled(d, has_travelled.indexOf(d[0]) === -1)));
                    yield parser.done();
                    let message;
            ...
            handle_command: venience_world_1.wrap_handler(function* (parser) {
                ...
                let read_consumer = venience_world_1.wrap_handler(function* (parser) {
                    ...
                    let read_0_consumer = venience_world_1.wrap_handler(function* (parser) {
                        yield parser.consume_option([
                            datatypes_1.annotate(['the', 'parchment'], {
                                display: parser_1.DisplayEltType.filler,
        ...
        (() => {
            ...
            return {
                ...
                handle_command: venience_world_1.wrap_handler(function* (parser) {
                    ...
                    let reify_consumer = venience_world_1.wrap_handler(function* (parser) {
                        yield parser.consume_option([
                            datatypes_1.annotate(['reify'], {
                                display: parser_1.DisplayEltType.keyword,
                                enabled: !finished
                            })
                        ]);
                        let opt1 = yield parser.consume_option([
                            datatypes_1.annotate(['the'], {
                                display: parser_1.DisplayEltType.filler,
                        ...
                        if (opt1 === 'the') {
                            opt2 = yield parser.consume_option([
                                datatypes_1.annotate(['calamity'], { enabled: !has_interpreted['the calamity'] }),
                                datatypes_1.annotate(['shattered', 'mirror'], { enabled: !has_interpreted['the shattered mirror'] }),
                                datatypes_1.annotate(['ice-covered', 'mountain'], { enabled: !has_interpreted['the ice-covered mountain'] })
                            ]);
                        }
                        else {
...
//# sourceMappingURL=01_chapter_1.js.map