...
2. **The parser re-runs the whole thread once per leaf branch** (`run_thread` restarts on every `split`), so cost is linear in the number of distinct commands. Narrascope peaks at ~15 threads (0–17 ms per command). A mapping grammar of 8 steps × 15 events + 7 roles × 20 participants is ~250 threads: probably fine, but 15× anything the engine has run, paid on every keystroke.
...
4. **The story tree must mirror the DOM one-to-one.** Updates reach the DOM by *path from the story root* (`dom_lookup_path`); `History` renders once and then mutates. A second rendering of the same frames (a board as "projection of history") cannot be animated; a board that *is* a story subtree can.
...
6. **The prompt lives inside the story hole**, which `reflect.tsx` already moves inside a frame (nested frames; `dist/history.css` styles `.story .frame .frame`). A prompt pinned outside the story changes `app.tsx`, `scroll_down` and the hole logic.

One play observation: the current demo prompts "you might remember something focused" *inside* a reflection where `remember` is locked, so you must `end reflection`, `remember`, and re-reflect. A lock must never hide the command the prompt just named.
...
**Beat 1, mapping.** Fifteen commands: seven role bindings plus eight step placements. In the campfire the nouns are literally *tinder*, *kindling*, *firewood*, so bindings are `the tinder is the tinder`. The incremental green/amber/red board makes the honest play pattern "pick, look at the colour, pick again": brute force with feedback. Here that is fine; the doc calls this one trivial (l. 246). But A's roles table has seven roles, and I cannot tell what the player binds *ember, flame, blaze, ash* to in the campfire. Roles the prose never names should be derived from the step placement, not typed.
...
The coda (mapping the Voice of Fire onto today's lesson) is clever and I would cut it: "ash = you don't really see it" is a wink, and the demo should end on the mutter.

**Playtime:** 45–70 minutes, of which perhaps 25 are transcription and role binding.

### Builder pass

**Reuse unchanged:** `world.tsx`, `puffer.ts`, `lock.ts`, `parser/`, `gist/`, `story/` (`graft` is exactly the retroactive annotation A needs). Correct as claimed.

**Extend:** `history.tsx` — but not as A says. "Frames render into columns keyed by `Event.seq`" is a second arrangement of the same nodes, which fact 4 forbids. The board must be a subtree of the story tree, rows as gist-bearing nodes so `S.has_gist(...)` updates animate them. A day's redesign, not a blocker.
...
Second risk: `supervenience.ts` as a test oracle. BFS over a ~250-command mapping grammar with no narrative dimensions is the explosion `supervenience_spec.ts` warns about. Use `traverse_thread` at scripted states; keep future search away from the board.
...
**Beat 5, house.** The burning lines (l. 336–342) are forced to `follows` because "no voice has been taught yet", and Katya says "we will need better voices for that". This is the most interesting design choice in B — a debt paid in the forest — and the only place any proposal turns the forest story into a *need*. Keep it.

**Beat 6–7.** Forest: five disembodied voices with dashed brackets and italic carats. Lovely to look at; as in A, nothing to decide. Wise man: fifteen rows, seven voices. **Most boring stretch in B:** those fifteen conversion rows before the literal mapping, each one "pick the unlocked verb". Then the punchline: `collapse the unmapped` folds thirteen rows into one bar — **B's best moment**, the visual form of "an awful lot of extra story" (l. 451). The second column, the apply toggle, and objections as commands are all right; the ending (solution II flickers, four barcode chips) lands.
...
B claims ~900–1,200 UI lines. I count more, and two of B's load-bearing claims are wrong about this engine:
...
Then the genuinely new work: **SVG lines** anchored by `getBoundingClientRect`, recomputed after every 700 ms animation stage, on collapse, and on resize — ~200 lines plus a `ResizeObserver`/rAF loop, and the first thing that will look wrong in a demo video. **This is B's riskiest piece.** The **pinned prompt** outside the hole touches `app.tsx`, `scroll_down`, and the hole-moving logic — moderate. The **voice carat** is trivial; **typeahead chips** are small; **ledger nesting** by moving the hole into a parent frame is exactly the `reflect.tsx` trick and B is right about it. The **mapping judge** is absent and must be written. The narrow layout I would drop.
...
**Brute force.** C has B's hole, worse: "Consistent sets are accepted" with no definition of consistent. Nudges are `when(story, mapping, pass)` closures, a per-story list. Nudge 7 rejects the literal ash in the figurative pass; nothing rejects `map the burning to "sing"`, or all eight steps to `travel`. Without a default rule the figurative solution "locks" after any eight maps. **Flag: two implementers would build different games**, one accepting everything not nudged, one hardcoding l. 471.

**Playtime:** 35–50 minutes, shortest of the three.

### Builder pass

C reuses the parts of the engine that are actually cheap: retroactive frame edits (`S.map_worlds(...)` is one line, what `reflect.tsx` does to unfocus), `replace_children` for recolouring, css ops for residue, `Locked` for the unsayable line, the `notes`/acquired-action pattern for voices, `interpretation_effects` staging so retroactive changes animate first. ~10–30 lines each. C draws lines as text (`----+--> line 9`), so no SVG, and its left column is naturally a story subtree, so fact 4 is satisfied.
...
1. **Skeleton: A's `Event` / `Sequence` / `AbstractSequence` / `Mapping` with rules L1–L5.** The only proposal with a judge, and the only one where "the player's transcript is already mappable" is true in the data.
...
3. **Replace A's free-form `participants` with an explicit per-story role-candidate table** (`role → [participant, pass?]`): the fudge factor becomes visible data, the figurative solution is a row with `pass: 'figurative'`, and two implementers get the same game. Derive ember/flame/blaze/ash from step placement; never make the player bind them.
...
6. **Every decision point gets a tempting wrong option with an authored nudge** (C's nudges 1–8, A's "The singing is not ash"). Under transparent discoverability the puzzle lives in the meaning of choosing wrong; write the nudges first.
...
9. **Apply has two visible consequences:** the Fire re-speaks the mapped story on the right (A); the left column is recoloured in the fire's terms, with strike-through residue if unmapped (C). Both are `replace_children`/css ops on gist-tagged nodes.
...
13. **Board = a persistent subtree of the story tree** (`.board` beside the frames), rows as gist-bearing nodes, two columns by CSS grid — the only way the existing staged/animated updates drive the board (fact 4). No second renderer of the same frames.
14. **No SVG lines in the first build.** Numbered colour badges on left rows and matching bands on right steps (A's ①, B's bands). Add Bézier lines as polish later; they are the piece most likely to be wrong on a first try (l. 154 expected that).
...
17. **Voice switching:** `speak as <voice>` as a command, a coloured voice bar, a voice-aware carat. Where the document admits more than one voice (house burning lines, forest), admit both with authored consequences; include C's `the Voice of Fire` trap. Mapping stays voice-indifferent (A's L5).
...
19. **Engine guards before content:** (a) a test that no step name collides with an event nominalisation (fact 3); (b) measure keystroke parse time on the wise-man grammar and, above ~30 ms, lead with a verb (`map`, `bind`) so threads die at token one; (c) test reachability of both wise-man solutions with `traverse_thread` at scripted states, not `search_future`.
