...
type WorldWithCutscenes<T extends WorldType<T>> = T & {
...
export function build_cutscene<T extends WorldType<T>>(data: CutsceneData): Cutscene<T> {
    return data.map(([cmd, message]) => ({
        command_name: split_tokens(cmd),
        execute: with_early_stopping(
            function*(world: WorldWithCutscenes<T>, parser: CommandParser){
                yield parser.done();
