/*
    ...
    is a class on a gist-addressed node (and, for the unmapped rows, one bar).
*/
...
import { board_story, event_frame, FireWorld, phrase, scene_of } from '../world';
...
function unmapped_count(w: FireWorld, story: StorySpec): number {
    const mapped = new Set(w.mappings.filter(m => m.sequence === story.id).flatMap(m => m.placements.map(p => p.event)));
    return (w.sequences[story.id]?.events.length ?? 0) - mapped.size;
}

function things(w: FireWorld): Thing[] {
    const result: Thing[] = [];
    if (w.scene !== 'classroom') {
    ...
    if (story !== undefined) {
        ...
        if (w.scene !== scene_of(story, 'transcribing')) {
        ...
        for (let n = 1; n <= transcribed; n++) {
            result.push({ name: names[n - 1], id: `${story.id}:${n}`, ops: c => event_ops(event_frame(w, story.id, n)!, c) });
        }
    }
